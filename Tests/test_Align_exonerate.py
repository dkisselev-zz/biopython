...
class Exonerate_est2genome(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m est2genome ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar yes --showvulgar no"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        assert alignment.score == 6150
        ...
        assert alignment.format("exonerate", "cigar") == """\
cigar: gi|296143771|ref|NM_001180731.1| 0 1230 + gi|330443520|ref|NC_001136.10| 1319275 1318045 - 6150 M 1230
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (1230 aligned letters; 0 identities; 0 mismatches; 0 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
...
"""
        ...
        assert counts.aligned == 1230
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443688|ref|NC_001145.3|"
        assert alignment.score == 439
        assert alignment.coordinates[0, 0] < alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (326 aligned letters; 0 identities; 0 mismatches; 387885 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 326:
...
"""
        ...
        assert counts.internal_deletions == 387865
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 387885
        assert counts.insertions == 20
        assert counts.deletions == 387865
        assert counts.gaps == 387885
        assert counts.aligned == 326
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443688|ref|NC_001145.3|"
        assert alignment.score == 263
        assert alignment.coordinates[0, 0] > alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates,
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (358 aligned letters; 0 identities; 0 mismatches; 118525 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 358:
...
"""
        ...
        assert counts.right_insertions == 0
        assert counts.right_deletions == 0
        assert counts.internal_insertions == 23
        assert counts.internal_deletions == 118502
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 118525
        assert counts.insertions == 23
        assert counts.deletions == 118502
        assert counts.gaps == 118525
        assert counts.aligned == 358
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m est2genome ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar no --showvulgar yes"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"M")
            assert alignment.format("exonerate", "vulgar") == """\
...
"""
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"MIMIMIMIMIM5N3MDMIMIMDMDM5N3MDMIMDMIMDMIMDMIM5N3MDM")
            assert alignment.format("exonerate", "vulgar") == """\
...
"""
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"MDMIMDMDMIMDMDMIMIM3N5MIMDMIMDMIMDM3N5MIMIMIMIMDMDMDMDM")
            assert alignment.format("exonerate", "vulgar") == """\
...
class Exonerate_affine_local(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m affine:local ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar yes --showvulgar no"
        assert alignments.metadata["Hostname"] == "Michiels-MacBook-Pro.local"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        assert alignment.score == 6150
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (450 aligned letters; 0 identities; 0 mismatches; 53 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 450:
...
"""
        ...
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 53
        assert counts.insertions == 19
        assert counts.deletions == 34
        assert counts.gaps == 53
        assert counts.aligned == 450
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443715|ref|NC_001146.8|"
        assert alignment.score == 219
        assert np.array_equal(
                alignment.coordinates,
                # fmt: off
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (436 aligned letters; 0 identities; 0 mismatches; 43 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 436:
...
"""
        ...
        assert counts.internal_deletions == 22
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 43
        assert counts.insertions == 21
        assert counts.deletions == 22
        assert counts.gaps == 43
        assert counts.aligned == 436
        with pytest.raises(StopIteration):
            next(alignments)
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m affine:local ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar no --showvulgar yes"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
...
"""
        ...
        if check_operations:
            assert alignment.operations == bytearray(
...
class Exonerate_cdna2genome(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m cdna2genome ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar yes --showvulgar no"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        assert alignment.score == 6146
        assert alignment.coordinates[0, 0] > alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
                # fmt: off
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (494 aligned letters; 0 identities; 0 mismatches; 581734 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 494:
...
"""
        ...
        assert counts.right_insertions == 0
        assert counts.right_deletions == 0
        assert counts.internal_insertions == 22
        assert counts.internal_deletions == 581712
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 581734
        assert counts.insertions == 22
        assert counts.deletions == 581712
        assert counts.gaps == 581734
        assert counts.aligned == 494
        with pytest.raises(StopIteration):
            next(alignments)
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m cdna2genome ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar no --showvulgar yes"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"MCM")
            assert alignment.format("exonerate", "vulgar") == """\
...
class Exonerate_coding2coding(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m coding2coding ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar yes --showvulgar no"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        assert alignment.score == 2151
        assert alignment.coordinates[0, 0] < alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] > alignment.coordinates[1, -1]
        assert np.array_equal(
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (1227 aligned letters; 0 identities; 0 mismatches; 0 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 1227:
...
"""
        ...
        assert counts.deletions == 0
        assert counts.gaps == 0
        assert counts.aligned == 1227
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        assert alignment.score == 2106
        assert alignment.coordinates[0, 0] > alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates, np.array([[1319275, 1318045], [0, 1230]])
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (156 aligned letters; 0 identities; 0 mismatches; 3 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 156:
...
"""
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m coding2coding ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar no --showvulgar yes"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"C")
            assert alignment.format("exonerate", "vulgar") == """\
...
"""
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"CIC")
            assert alignment.format("exonerate", "vulgar") == """\
...
class Exonerate_coding2genome(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m coding2genome ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar yes --showvulgar no"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
...
"""
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m coding2genome ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar no --showvulgar yes"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
...
class Exonerate_dna2protein(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate --showcigar yes --showvulgar no --showalignment no nuc2.fa pro.fa"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "dna"
        assert alignment.target.id == "protein"
        assert alignment.score == 105
        assert alignment.coordinates[0, 0] < alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(alignment.coordinates, np.array([[313, 344], [0, 93]]))
        assert alignment.target.annotations["molecule_type"] == "protein"
        assert alignment.format("exonerate", "cigar") == """\
...
"""
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate --showcigar no --showvulgar yes --showalignment no nuc2.fa pro.fa"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "dna"
        assert alignment.target.id == "protein"
...
class Exonerate_genome2genome(unittest.TestCase):
    ...
    def check_cigar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m genome2genome ../intron.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showvulgar yes --showcigar yes"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "sacCer3_dna"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        assert alignment.score == 2641
        assert alignment.coordinates[0, 0] > alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] > alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates,
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (529 aligned letters; 0 identities; 0 mismatches; 0 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 529:
...
"""
        ...
        alignment = next(alignments)
        assert alignment.query.id == "sacCer3_dna"
        assert alignment.target.id == "gi|330443489|ref|NC_001135.5|"
        assert alignment.score == 267
        assert alignment.coordinates[0, 0] < alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] > alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates,
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (323 aligned letters; 0 identities; 0 mismatches; 91584 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 323:
...
"""
        ...
        assert counts.internal_deletions == 91578
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 91584
        assert counts.insertions == 6
        assert counts.deletions == 91578
        assert counts.gaps == 91584
        assert counts.aligned == 323
        alignment = next(alignments)
        assert alignment.query.id == "sacCer3_dna"
        assert alignment.target.id == "gi|330443489|ref|NC_001135.5|"
        assert alignment.score == 267
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"M5N3MDMDM5N3MIMDMDMDMS5N3SCMIM5N3MDMIMDMDMDMDMIMDMDM")
        assert alignment.format("exonerate", "cigar") == """\
...
"""
        ...
        alignment = next(alignments)
        assert alignment.query.id == "sacCer3_dna"
        assert alignment.target.id == "gi|330443667|ref|NC_001143.9|"
        assert alignment.score == 267
        assert alignment.coordinates[0, 0] > alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] > alignment.coordinates[1, -1]
        assert np.array_equal(
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (299 aligned letters; 0 identities; 0 mismatches; 569730 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 299:
...
"""
        ...
        assert counts.right_insertions == 0
        assert counts.right_deletions == 0
        assert counts.internal_insertions == 152
        assert counts.internal_deletions == 569578
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 569730
        assert counts.insertions == 152
        assert counts.deletions == 569578
        assert counts.gaps == 569730
        assert counts.aligned == 299
        alignment = next(alignments)
        assert alignment.query.id == "sacCer3_dna"
        assert alignment.target.id == "gi|330443667|ref|NC_001143.9|"
        assert alignment.score == 267
...
"""
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m genome2genome ../intron.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showvulgar yes --showcigar no"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "sacCer3_dna"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
...
class Exonerate_ungapped(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m ungapped ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar yes --showvulgar no"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (115 aligned letters; 0 identities; 0 mismatches; 0 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 115:
...
"""
        ...
        assert counts.deletions == 0
        assert counts.gaps == 0
        assert counts.aligned == 115
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443688|ref|NC_001145.3|"
        assert alignment.score == 151
        assert alignment.coordinates[0, 0] < alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (68 aligned letters; 0 identities; 0 mismatches; 0 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 68:
...
"""
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m ungapped ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar no --showvulgar yes"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
...
class Exonerate_ungapped_trans(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m ungapped:trans ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar yes --showvulgar no"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
...
"""
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m ungapped:trans ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar no --showvulgar yes"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
...
class Exonerate_ner(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m ner ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar yes --showvulgar no"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
...
"""
        ...
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443681|ref|NC_001144.5|"
        assert alignment.score == 502
        assert alignment.coordinates[0, 0] < alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates,
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (445 aligned letters; 0 identities; 0 mismatches; 21314 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 445:
...
"""
        ...
        assert counts.right_insertions == 0
        assert counts.right_deletions == 0
        assert counts.internal_insertions == 675
        assert counts.internal_deletions == 20639
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 21314
        assert counts.insertions == 675
        assert counts.deletions == 20639
        assert counts.gaps == 21314
        assert counts.aligned == 445
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        assert alignment.score == 440
        assert alignment.coordinates[0, 0] < alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates,
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (404 aligned letters; 0 identities; 0 mismatches; 532 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 404:
...
"""
        ...
        assert counts.right_insertions == 0
        assert counts.right_deletions == 0
        assert counts.internal_insertions == 279
        assert counts.internal_deletions == 253
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 532
        assert counts.insertions == 279
        assert counts.deletions == 253
        assert counts.gaps == 532
        assert counts.aligned == 404
        with pytest.raises(StopIteration):
            next(alignments)
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m ner ../scer_cad1.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar no --showvulgar yes"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (413 aligned letters; 0 identities; 0 mismatches; 21378 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 413:
...
"""
        ...
        assert counts.right_insertions == 0
        assert counts.right_deletions == 0
        assert counts.internal_insertions == 707
        assert counts.internal_deletions == 20671
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 21378
        assert counts.insertions == 707
        assert counts.deletions == 20671
        assert counts.gaps == 21378
        assert counts.aligned == 413
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        assert alignment.score == 440
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (381 aligned letters; 0 identities; 0 mismatches; 578 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 381:
...
"""
        ...
        assert counts.right_insertions == 0
        assert counts.right_deletions == 0
        assert counts.internal_insertions == 302
        assert counts.internal_deletions == 276
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 578
        assert counts.insertions == 302
        assert counts.deletions == 276
        assert counts.gaps == 578
        assert counts.aligned == 381
        with pytest.raises(StopIteration):
            next(alignments)
...
class Exonerate_multiple(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m est2genome comb.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar yes --showvulgar no"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296142823|ref|NM_001178508.1|"
        assert alignment.target.id == "gi|330443482|ref|NC_001134.8|"
        assert alignment.score == 4485
        assert alignment.coordinates[0, 0] < alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (897 aligned letters; 0 identities; 0 mismatches; 0 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 897:
...
"""
        ...
        assert counts.deletions == 0
        assert counts.gaps == 0
        assert counts.aligned == 897
        alignment = next(alignments)
        assert alignment.query.id == "gi|296142823|ref|NM_001178508.1|"
        assert alignment.target.id == "gi|330443753|ref|NC_001148.4|"
        assert alignment.score == 941
        assert alignment.coordinates[0, 0] > alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates,
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (869 aligned letters; 0 identities; 0 mismatches; 56 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 869:
...
"""
        ...
        assert counts.right_insertions == 0
        assert counts.right_deletions == 0
        assert counts.internal_insertions == 25
        assert counts.internal_deletions == 31
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 56
        assert counts.insertions == 25
        assert counts.deletions == 31
        assert counts.gaps == 56
        assert counts.aligned == 869
        alignment = next(alignments)
        assert alignment.query.id == "gi|296142823|ref|NM_001178508.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        assert alignment.score == 651
        assert alignment.coordinates[0, 0] < alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates,
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (641 aligned letters; 0 identities; 0 mismatches; 32313 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 641:
...
"""
        ...
        assert counts.right_insertions == 0
        assert counts.right_deletions == 0
        assert counts.internal_insertions == 46
        assert counts.internal_deletions == 32267
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 32313
        assert counts.insertions == 46
        assert counts.deletions == 32267
        assert counts.gaps == 32313
        assert counts.aligned == 641
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        assert alignment.score == 6150
...
"""
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m est2genome comb.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showvulgar yes"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296142823|ref|NM_001178508.1|"
        assert alignment.target.id == "gi|330443482|ref|NC_001134.8|"
...
class Exonerate_coding2coding_fshifts(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m coding2coding c2c_frameshift2.fa scer_cad1.fa --showcigar yes --showvulgar no --showalignment no --bestn 3"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.score == 213
        assert alignment.coordinates[0, 0] < alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates,
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (159 aligned letters; 0 identities; 0 mismatches; 7 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 159:
...
"""
        ...
        assert counts.aligned == 159
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.score == 201
        assert alignment.coordinates[0, 0] > alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] > alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates,
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (156 aligned letters; 0 identities; 0 mismatches; 7 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 156:
...
"""
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m coding2coding c2c_frameshift2.fa scer_cad1.fa --showvulgar yes --showalignment no --bestn 3"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.target.id == "gi|296143771|ref|NM_001180731.1|"
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"CFCFCFC")
            assert alignment.format("exonerate", "vulgar") == """\
...
"""
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"CDCFC")
            assert alignment.format("exonerate", "vulgar") == """\
...
class Exonerate_protein2dna(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m protein2dna ../scer_cad1_prot.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar yes --showvulgar no"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "sp|P24813|YAP2_YEAST"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
        assert alignment.score == 2105
        assert alignment.coordinates[0, 0] > alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (276 aligned letters; 0 identities; 0 mismatches; 3 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 276:
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (156 aligned letters; 0 identities; 0 mismatches; 1 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 156:
...
"""
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m protein2dna ../scer_cad1_prot.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar no --showvulgar yes"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "sp|P24813|YAP2_YEAST"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
...
"""
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"MDM")
        assert alignment.format("exonerate", "vulgar") == """\
...
"""
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"MIM")
        assert alignment.format("exonerate", "vulgar") == """\
...
class Exonerate_protein2dna_fshifts(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m protein2dna scer_cad1_prot.fa scer_cad1frameshift.fa --showcigar yes --showvulgar no --showalignment no --bestn 3"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "sp|P24813|YAP2_YEAST"
        assert alignment.target.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.score == 367
        assert alignment.coordinates[0, 0] < alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates,
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (237 aligned letters; 0 identities; 0 mismatches; 2 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 237:
...
"""
        ...
        assert counts.aligned == 237
        alignment = next(alignments)
        assert alignment.query.id == "sp|P24813|YAP2_YEAST"
        assert alignment.target.id == "gi|296143771|ref|NM_001180731.1|"
        assert alignment.score == 322
        assert alignment.coordinates[0, 0] < alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(alignment.coordinates, np.array([[16, 208], [6, 70]]))
        assert alignment.format("exonerate", "cigar") == """\
cigar: sp|P24813|YAP2_YEAST 6 70 . gi|296143771|ref|NM_001180731.1| 16 208 + 322 M 192
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (192 aligned letters; 0 identities; 0 mismatches; 0 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 192:
...
"""
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m protein2dna scer_cad1_prot.fa scer_cad1frameshift.fa --showvulgar yes --showalignment no --bestn 3"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "sp|P24813|YAP2_YEAST"
        assert alignment.target.id == "gi|296143771|ref|NM_001180731.1|"
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"MFM")
            assert alignment.format("exonerate", "vulgar") == """\
...
class Exonerate_protein2genome(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m protein2genome ../scer_cad1_prot.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar yes --showvulgar no"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "sp|P24813|YAP2_YEAST"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
...
"""
        ...
        alignment = next(alignments)
        assert alignment.query.id == "sp|P24813|YAP2_YEAST"
        assert alignment.target.id == "gi|330443590|ref|NC_001140.6|"
        assert alignment.score == 122
        assert alignment.coordinates[0, 0] > alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates,
        ...
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (262 aligned letters; 0 identities; 0 mismatches; 15934 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 262:
...
"""
        ...
        assert counts.right_deletions == 0
        assert counts.internal_insertions == 0
        assert counts.internal_deletions == 15934
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 15934
        assert counts.insertions == 0
        assert counts.deletions == 15934
        assert counts.gaps == 15934
        assert counts.aligned == 262
        with pytest.raises(StopIteration):
            next(alignments)
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m protein2genome ../scer_cad1_prot.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showalignment no --showcigar no --showvulgar yes"
        assert alignments.metadata["Hostname"] == "blackbriar"
        alignment = next(alignments)
        assert alignment.query.id == "sp|P24813|YAP2_YEAST"
        assert alignment.target.id == "gi|330443520|ref|NC_001136.10|"
...
"""
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"MS5N3SM")
        assert alignment.format("exonerate", "vulgar") == """\
...
class Exonerate_protein2genome_revcomp_fshifts(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m protein2genome gene026_baits.fasta gene026_contigs.fasta --showalignment no --showcigar yes --showvulgar no --bestn 2 --refine full"
        assert alignments.metadata["Hostname"] == "Michiels-MacBook-Pro.local"
        alignment = next(alignments)
        assert alignment.query.id == "Morus-gene026"
        assert alignment.target.id == "NODE_2_length_1708_cov_48.590765"
        assert alignment.score == 1308
        assert alignment.coordinates[0, 0] > alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates,
...
"""
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (1077 aligned letters; 0 identities; 0 mismatches; 21 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 1077:
...
"""
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m protein2genome gene026_baits.fasta gene026_contigs.fasta --showalignment no --showcigar no --showvulgar yes --bestn 2 --refine full"
        assert alignments.metadata["Hostname"] == "Michiels-MacBook-Pro.local"
        alignment = next(alignments)
        assert alignment.query.id == "Morus-gene026"
        assert alignment.target.id == "NODE_2_length_1708_cov_48.590765"
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"MDMIMIMIMIFM")
            assert alignment.format("exonerate", "vulgar") == """\
...
class Exonerate_protein2genome_met_intron(unittest.TestCase):
    ...
    def check_cigar(self, alignments):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m protein2genome gene001_baits.fasta gene001_contigs.fasta --showalignment no --showcigar yes --showvulgar no --bestn 1 --refine full"
        assert alignments.metadata["Hostname"] == "Michiels-MacBook-Pro.local"
        alignment = next(alignments)
        assert alignment.query.id == "Morus-gene001"
        assert alignment.target.id == "NODE_1_length_2817_cov_100.387732"
        assert alignment.score == 1978
        assert alignment.coordinates[0, 0] > alignment.coordinates[0, -1]
        assert alignment.coordinates[1, 0] < alignment.coordinates[1, -1]
        assert np.array_equal(
                alignment.coordinates,
        ...
        counts = alignment.counts()
        assert (repr(counts) == "<AlignmentCounts object (1301 aligned letters; 0 identities; 0 mismatches; 703 gaps) at 0x%x>"
            % id(counts))
        assert str(counts) == """\
AlignmentCounts object with
    aligned = 1301:
...
"""
        ...
        assert counts.right_deletions == 0
        assert counts.internal_insertions == 0
        assert counts.internal_deletions == 703
        assert counts.left_gaps == 0
        assert counts.right_gaps == 0
        assert counts.internal_gaps == 703
        assert counts.insertions == 0
        assert counts.deletions == 703
        assert counts.gaps == 703
        assert counts.aligned == 1301
        with pytest.raises(StopIteration):
            next(alignments)
    ...
    def check_vulgar(self, alignments, check_operations=True):
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m protein2genome gene001_baits.fasta gene001_contigs.fasta --showalignment no --showcigar no --showvulgar yes --bestn 1 --refine full"
        assert alignments.metadata["Hostname"] == "Michiels-MacBook-Pro.local"
        alignment = next(alignments)
        assert alignment.query.id == "Morus-gene001"
        assert alignment.target.id == "NODE_1_length_2817_cov_100.387732"
        ...
        if check_operations:
            assert alignment.operations == bytearray(b"M5N3M5N3M5N3MS5N3SM5N3M")
        assert alignment.format("exonerate", "vulgar") == """\
...
class Exonerate_none(unittest.TestCase):
    def test_exn_22_q_none(self):
        ...
        alignments = Align.parse(exn_file, "exonerate")
        assert alignments.metadata["Program"] == "exonerate"
        assert alignments.metadata["Command line"] == "exonerate -m est2genome none.fa /media/Waterloo/Downloads/genomes/scer_s288c/scer_s288c.fa --bestn 3 --showcigar yes --showvulgar yes"
        assert alignments.metadata["Hostname"] == "blackbriar"
        with pytest.raises(StopIteration):
            next(alignments)
        alignments = iter(alignments)
