...
class ModTest(unittest.TestCase):
    ...
    def testAlignmentExists(self):
        with pytest.raises((EnvironmentError, IOError)):
            codeml.Codeml(alignment="nonexistent")
    ...
    def testTreeExists(self):
        with pytest.raises((EnvironmentError, IOError)):
            codeml.Codeml(tree="nonexistent")
        self.cml.alignment = self.align_file
    ...
    def testPamlErrorsCaught(self):
        ...
        self.cml.out_file = self.out_file
        with pytest.raises((EnvironmentError, PamlError)):
            self.cml.run()
    ...
    def testCtlFileValidOnRead(self):
        ...
        assert sorted(self.cml._options) == sorted(target_options)
        for key in target_options:
            assert self.cml._options[key] == target_options[key], f"{key}: {self.cml._options[key]!r} vs {target_options[key]!r}"
    ...
    def testParseSEs(self):
        ...
        for results_file in os.listdir(res_dir):
            ...
            assert len(results) == 4, version_msg
            assert "NSsites" in results, version_msg
            models = results["NSsites"]
            # Only site class model 0 was simulated
            assert len(models) == 1, version_msg
            assert 0 in models, version_msg
            model = models[0]
            assert len(model) == 5, version_msg
            assert "parameters" in model, version_msg
            params = model["parameters"]
            # There should be one new item in the parameters, "SEs"
            assert len(params) == SITECLASS_PARAMS[0] + 1, version_msg
            assert "SEs" in params, version_msg
    ...
    def testParseAllNSsites(self):
        ...
        for results_file in os.listdir(res_dir):
            ...
            assert len(results["NSsites"]) == 6, version_msg
            ...
            for model_num in [0, 1, 2, 3, 7, 8]:
                ...
                assert "parameters" in model, version_msg
                params = model["parameters"]
                assert len(params) == SITECLASS_PARAMS[model_num], version_msg
                assert "branches" in params, version_msg
                branches = params["branches"]
                # There are 7 branches in the test case (specific to these
                # test cases)
                assert len(branches) == 7, version_msg
                if "site classes" in params:
                    assert len(params["site classes"]) == SITECLASSES[model_num], version_msg
    ...
    def testParseNSsite3(self):
        ...
        for results_file in os.listdir(res_dir):
            ...
            assert len(results) == 5, version_msg
            assert "site-class model" in results, version_msg
            assert results["site-class model"] == "discrete", version_msg
            assert "NSsites" in results, version_msg
            # There should be 1 NSsites class: 3
            assert len(results["NSsites"]) == 1, version_msg
            # Each site class model should have 5 sub-items: 'lnL', 'tree',
            # 'description', 'parameters', & 'tree length'. It should
            # have the correct number of parameters also.
            ...
            assert "parameters" in model, version_msg
            params = model["parameters"]
            assert len(params) == SITECLASS_PARAMS[3], version
            assert "site classes" in params, version_msg
            site_classes = params["site classes"]
            assert len(site_classes) == 4, version_msg
    ...
    def testParseBranchSiteA(self):
        ...
        for results_file in os.listdir(res_dir):
            ...
            assert len(models) == 1, version_msg
            assert 2 in models, version_msg
            model = models[2]
            assert len(model) == 5, version_msg
            assert "parameters" in model, version_msg
            params = model["parameters"]
            # Branch Site A results lack a "branches" parameter
            assert len(params) == SITECLASS_PARAMS[2] - 1, version_msg
            assert "site classes" in params, version_msg
            site_classes = params["site classes"]
            # Branch Site A adds another site class
            assert len(site_classes) == SITECLASSES[2] + 1, version
            for class_num in [0, 1, 2, 3]:
                assert class_num in site_classes, version_msg
                site_class = site_classes[class_num]
                assert len(site_class) == 2, version_msg
                assert "branch types" in site_class, version_msg
                branches = site_class["branch types"]
                assert len(branches) == 2, version_msg
    ...
    def testParseCladeModelC(self):
        ...
        for results_file in os.listdir(cladeC_res_dir):
            ...
            assert "site classes" in params, version_msg
            site_classes = params["site classes"]
            assert len(site_classes) == SITECLASSES[2], version
    ...
    def testParseNgene2Mgene02(self):
        ...
        for results_file in os.listdir(res_dir):
            ...
            assert len(params) == 4, version_msg
            assert "rates" in params, version_msg
            rates = params["rates"]
            assert len(rates) == 2, version_msg
    ...
    def testParseNgene2Mgene1(self):
        ...
        for results_file in os.listdir(res_dir):
            ...
            results = codeml.read(results_path)
            assert len(results) == 4, version_msg
            assert "genes" in results, version_msg
            genes = results["genes"]
            assert len(genes) == 2, version_msg
            model = genes[0]
            assert len(model) == 5, version_msg
            assert "parameters" in model, version_msg
            params = model["parameters"]
            assert len(params) == SITECLASS_PARAMS[0], version_msg
    ...
    def testParseNgene2Mgene34(self):
        ...
        for results_file in os.listdir(res_dir):
            ...
            params = model["parameters"]
            # This type of model has fewer parameters for model 0
            assert len(params) == 3, version_msg
            assert "rates" in params, version_msg
            rates = params["rates"]
            assert len(rates) == 2, version_msg
            assert "genes" in params, version_msg
            genes = params["genes"]
            assert len(genes) == 2, version_msg
    ...
    def testParseFreeRatio(self):
        ...
        for results_file in os.listdir(res_dir):
            ...
            assert len(model) == 8, version_msg
            assert "parameters" in model, version_msg
            params = model["parameters"]
            assert len(params) == SITECLASS_PARAMS[0], version_msg
            assert "branches" in params, version_msg
            ...
            branches = params["branches"]
            assert len(branches) == 7, version_msg
            assert "omega" in params, version_msg
            omega = params["omega"]
            assert len(omega) == 7, version_msg
    ...
    def testParsePairwise(self):
        ...
        for results_file in os.listdir(res_dir):
            ...
            assert len(results) == 5, version_msg
            assert "pairwise" in results, version_msg
            pairwise = results["pairwise"]
            assert len(pairwise) >= 2, version_msg + ": should have at least two sequences"
            for seq1, seq2 in itertools.combinations(pairwise.keys(), 2):
                assert len(pairwise[seq1][seq2]) == 7, version_msg + ": wrong number of parameters parsed"
                assert len(pairwise[seq2][seq1]) == 7, version_msg + ": wrong number of parameters parsed"
    ...
    def testParseSitesParamsForPairwise(self):
        ...
        for results_file in os.listdir(res_dir):
            ...
            assert len(seqs) >= 2, version_msg + ": should have at least two sequences"
            for seq1, seq2 in itertools.combinations(seqs, 2):
                params = results["pairwise"][seq1][seq2]
                assert len(params) == 7, (version_msg
                    + ": wrong number of parsed parameters"
                    + f" for {seq1}-{seq2}")
                for param in ("t", "S", "N", "omega", "dN", "dS", "lnL"):
                    assert param in params, version_msg + f": '{param}' not in parsed parameters"
    ...
    def testParseAA(self):
        ...
        for results_file in os.listdir(res_dir):
            ...
            if version == "4_1":
                assert len(results) == 4, version_msg
                assert "lnL max" in results, version_msg
            else:
                assert len(results) == 5, version_msg
                assert "lnL max" in results, version_msg
                assert "distances" in results, version_msg
                distances = results["distances"]
                # non-pairwise AA analysis only gives raw distances
                assert len(distances) == 1, version_msg
    ...
    def testParseAAPairwise(self):
        ...
        for results_file in os.listdir(res_dir):
            ...
            assert len(distances) == 2, version_msg
    ...
    def testTreeParseVersatility(self):
        ...
        m0 = nssites[0]
        assert "tree" in m0
        assert m0["tree"] is not None
        assert len(m0["tree"]) != 0
    ...
    def testParseM2arel(self):
        ...
        for results_file in os.listdir(res_dir):
            ...
            results = codeml.read(results_path)
            assert "NSsites" in results
            assert 22 in results["NSsites"]
            model = results["NSsites"][22]
            assert len(model) == 5, version_msg
            params = model["parameters"]
            assert len(params) == SITECLASS_PARAMS[22], version_msg
