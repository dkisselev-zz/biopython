...
class TestProdocRead(unittest.TestCase):
    ...
    def test_read_pdoc00100(self):
        ...
        assert record.accession == "PDOC00100"
        assert len(record.prosite_refs) == 4
        assert record.prosite_refs[0] == ("PS00107", "PROTEIN_KINASE_ATP")
        assert record.prosite_refs[1] == ("PS00108", "PROTEIN_KINASE_ST")
        assert record.prosite_refs[2] == ("PS00109", "PROTEIN_KINASE_TYR")
        assert record.prosite_refs[3] == ("PS50011", "PROTEIN_KINASE_DOM")
        assert record.text == """\
******************************************
...
"""
        ...
        assert record.references[0].number == "1"
        assert record.references[0].authors == "Hanks S.K., Hunter T."
        assert record.references[0].citation == """\
...
PubMed=7768349"""
        assert record.references[1].number == "2"
        assert record.references[1].authors == "Hunter T."
        assert record.references[1].citation == """\
"Protein kinase classification."
Methods Enzymol. 200:3-37(1991).
PubMed=1835513"""
        assert record.references[2].number == "3"
        assert record.references[2].authors == "Hanks S.K., Quinn A.M."
        assert record.references[2].citation == """\
...
PubMed=1956325"""
        assert record.references[3].number == "4"
        assert record.references[3].authors == "Hanks S.K."
        assert record.references[3].citation == "Curr. Opin. Struct. Biol. 1:369-383(1991)."
        assert record.references[4].number == "5"
        assert record.references[4].authors == "Hanks S.K., Quinn A.M., Hunter T."
        assert record.references[4].citation == """\
"The protein kinase family: conserved features and deduced phylogeny
of the catalytic domains."
Science 241:42-52(1988).
PubMed=3291115"""
        assert record.references[5].number == "6"
        assert record.references[5].authors == "Knighton D.R., Zheng J.H., Ten Eyck L.F., Ashford V.A., Xuong N.-H., Taylor S.S., Sowadski J.M."
        assert record.references[5].citation == """\
...
PubMed=1862342"""
        assert record.references[6].number == "7"
        assert record.references[6].authors == "Bairoch A., Claverie J.-M."
        assert record.references[6].citation == """\
"Sequence patterns in protein kinases."
Nature 331:22-22(1988).
PubMed=3340146; DOI=10.1038/331022a0"""
        assert record.references[7].number == "8"
        assert record.references[7].authors == "Benner S."
        assert record.references[7].citation == "Nature 329:21-21(1987)."
        assert record.references[8].number == "9"
        assert record.references[8].authors == "Kirby R."
        assert record.references[8].citation == """\
"Evolutionary origin of aminoglycoside phosphotransferase resistance
genes."
J. Mol. Evol. 30:489-492(1990).
PubMed=2165531"""
        assert record.references[9].number == "10"
        assert record.references[9].authors == "Littler E., Stuart A.D., Chee M.S."
        assert record.references[9].citation == "Nature 358:160-162(1992)."
        assert record.references[10].number == "11"
        assert record.references[10].authors == "Munoz-Dorado J., Inouye S., Inouye M."
        assert record.references[10].citation == "Cell 67:995-1006(1991)."
    ...
    def test_read_pdoc00113(self):
        ...
        assert record.accession == "PDOC00113"
        assert len(record.prosite_refs) == 1
        assert record.prosite_refs[0] == ("PS00123", "ALKALINE_PHOSPHATASE")
        assert record.text == """\
************************************
...
"""
        ...
        assert record.references[0].number == "1"
        assert record.references[0].authors == "Trowsdale J., Martin D., Bicknell D., Campbell I."
        assert record.references[0].citation == """\
"Alkaline phosphatases."
Biochem. Soc. Trans. 18:178-180(1990).
PubMed=2379681"""
        assert record.references[1].number == "2"
        assert record.references[1].authors == "Manes T., Glade K., Ziomek C.A., Millan J.L."
        assert record.references[1].citation == """\
...
PubMed=2286375"""
        assert record.references[2].number == "3"
        assert record.references[2].authors == "Mansouri K., Piepersberg W."
        assert record.references[2].citation == """\
...
PubMed=1654502"""
    ...
    def test_read_pdoc00144(self):
        ...
        assert record.accession == "PDOC00144"
        assert len(record.prosite_refs) == 2
        assert record.prosite_refs[0] == ("PS00159", "ALDOLASE_KDPG_KHG_1")
        assert record.prosite_refs[1] == ("PS00160", "ALDOLASE_KDPG_KHG_2")
        assert record.text == """\
*************************************************
...
"""
        ...
        assert record.references[0].number == "1"
        assert record.references[0].authors == "Vlahos C.J., Dekker E.E."
        assert record.references[0].citation == """\
...
PubMed=3136164"""
    ...
    def test_read_pdoc00149(self):
        ...
        assert record.accession == "PDOC00149"
        assert len(record.prosite_refs) == 1
        assert record.prosite_refs[0] == ("PS00165", "DEHYDRATASE_SER_THR")
        assert record.text == """\
*********************************************************************
...
"""
        ...
        assert record.references[0].number == "1"
        assert record.references[0].authors == "Ogawa H., Gomi T., Konishi K., Date T., Nakashima H., Nose K., Matsuda Y., Peraino C., Pitot H.C., Fujioka M."
        assert record.references[0].citation == """\
...
PubMed=2674117"""
        assert record.references[1].number == "2"
        assert record.references[1].authors == "Datta P., Goss T.J., Omnaas J.R., Patil R.V."
        assert record.references[1].citation == """\
...
PubMed=3540965"""
        assert record.references[2].number == "3"
        assert record.references[2].authors == "Parsot C."
        assert record.references[2].citation == """\
...
PubMed=3098560"""
        assert record.references[3].number == "4"
        assert record.references[3].authors == "Grabowski R., Hofmeister A.E.M., Buckel W."
        assert record.references[3].citation == """\
...
PubMed=8236444"""
    ...
    def test_read_pdoc00340(self):
        ...
        assert record.accession == "PDOC00340"
        assert len(record.prosite_refs) == 3
        assert record.prosite_refs[0] == ("PS00406", "ACTINS_1")
        assert record.prosite_refs[1] == ("PS00432", "ACTINS_2")
        assert record.prosite_refs[2] == ("PS01132", "ACTINS_ACT_LIKE")
        assert record.text == """\
*********************
* Actins signatures *
*********************
...
"""
        ...
        assert record.references[0].number == "1"
        assert record.references[0].authors == "Sheterline P., Clayton J., Sparrow J.C."
        assert record.references[0].citation == "(In) Actins, 3rd Edition, Academic Press Ltd, London, (1996)."
        assert record.references[1].number == "2"
        assert record.references[1].authors == "Pollard T.D., Cooper J.A."
        assert record.references[1].citation == "Annu. Rev. Biochem. 55:987-1036(1986)."
        assert record.references[2].number == "3"
        assert record.references[2].authors == "Pollard T.D."
        assert record.references[2].citation == """\
"Actin."
Curr. Opin. Cell Biol. 2:33-40(1990).
PubMed=2183841"""
        assert record.references[3].number == "4"
        assert record.references[3].authors == "Rubenstein P.A."
        assert record.references[3].citation == """\
"The functional importance of multiple actin isoforms."
BioEssays 12:309-315(1990).
PubMed=2203335"""
        assert record.references[4].number == "5"
        assert record.references[4].authors == "Meagher R.B., McLean B.G."
        assert record.references[4].citation == "Cell Motil. Cytoskeleton 16:164-166(1990)."
    ...
    def test_read_pdoc00424(self):
        ...
        assert record.accession == "PDOC00424"
        assert len(record.prosite_refs) == 1
        assert record.prosite_refs[0] == ("PS00488", "PAL_HISTIDASE")
        assert record.text == """\
**********************************************************
...
"""
        ...
        assert len(record.references) == 4
        assert record.references[0].number == "1"
        assert record.references[0].authors == "Taylor R.G., Lambert M.A., Sexsmith E., Sadler S.J., Ray P.N., Mahuran D.J., McInnes R.R."
        assert record.references[0].citation == """\
...
PubMed=2120224"""
        assert record.references[1].number == "2"
        assert record.references[1].authors == "Langer M., Reck G., Reed J., Retey J."
        assert record.references[1].citation == """\
...
PubMed=8204579"""
        assert record.references[2].number == "3"
        assert record.references[2].authors == "Schuster B., Retey J."
        assert record.references[2].citation == """\
...
PubMed=8050576"""
        assert record.references[3].number == "4"
        assert record.references[3].authors == "Taylor R.G., McInnes R.R."
        assert record.references[3].citation == """\
...
PubMed=7961661"""
    ...
    def test_read_pdoc00472(self):
        ...
        assert record.accession == "PDOC00472"
        assert len(record.prosite_refs) == 1
        assert record.prosite_refs[0] == ("PS00546", "CYSTEINE_SWITCH")
        assert record.text == """\
*****************************
...
"""
        ...
        assert len(record.references) == 5
        assert record.references[0].number == "1"
        assert record.references[0].authors == "Woessner J.F. Jr."
        assert record.references[0].citation == """\
...
PubMed=1850705"""
        assert record.references[1].number == "2"
        assert record.references[1].authors == "Sanchez-Lopez R., Nicholson R., Gesnel M.C., Matrisian L.M., Breathnach R."
        assert record.references[1].citation == "J. Biol. Chem. 263:11892-11899(1988)."
        assert record.references[2].number == "3"
        assert record.references[2].authors == "Park A.J., Matrisian L.M., Kells A.F., Pearson R., Yuan Z.Y., Navre M."
        assert record.references[2].citation == """\
...
PubMed=1988438"""
        assert record.references[3].number == "4"
        assert record.references[3].authors == "Lepage T., Gache C."
        assert record.references[3].citation == """\
...
PubMed=2167841"""
        assert record.references[4].number == "5"
        assert record.references[4].authors == "Kinoshita T., Fukuzawa H., Shimada T., Saito T., Matsuda Y."
        assert record.references[4].citation == """\
...
PubMed=1584806"""
    ...
    def test_read_pdoc00640(self):
        ...
        assert record.accession == "PDOC00640"
        assert len(record.prosite_refs) == 1
        assert record.prosite_refs[0] == ("PS00812", "GLYCOSYL_HYDROL_F8")
        assert record.text == """\
******************************************
...
"""
        ...
        assert len(record.references) == 6
        assert record.references[0].number == "1"
        assert record.references[0].authors == "Beguin P."
        assert record.references[0].citation == """\
...
PubMed=2252383; DOI=10.1146/annurev.mi.44.100190.001251"""
        assert record.references[1].number == "2"
        assert record.references[1].authors == "Gilkes N.R., Henrissat B., Kilburn D.G., Miller R.C. Jr., Warren R.A.J."
        assert record.references[1].citation == """\
...
PubMed=1886523"""
        assert record.references[2].number == "3"
        assert record.references[2].authors == "Henrissat B., Claeyssens M., Tomme P., Lemesle L., Mornon J.-P."
        assert record.references[2].citation == """\
"Cellulase families revealed by hydrophobic cluster analysis."
Gene 81:83-95(1989).
PubMed=2806912"""
        assert record.references[3].number == "4"
        assert record.references[3].authors == "Henrissat B."
        assert record.references[3].citation == """\
...
PubMed=1747104"""
        assert record.references[4].number == "5"
        assert record.references[4].authors == "Alzari P.M., Souchon H., Dominguez R."
        assert record.references[4].citation == """\
...
PubMed=8805535"""
        assert record.references[5].number == "E1"
        assert record.references[5].authors == ""
        assert record.references[5].citation == "http://www.expasy.org/cgi-bin/lists?glycosid.txt"
    ...
    def test_read_pdoc00787(self):
        ...
        assert record.accession == "PDOC00787"
        assert len(record.prosite_refs) == 1
        assert record.prosite_refs[0] == ("PS01027", "GLYCOSYL_HYDROL_F39")
        assert record.text == """\
******************************************************
...
"""
        ...
        assert len(record.references) == 3
        assert record.references[0].number == "1"
        assert record.references[0].authors == "Henrissat B., Bairoch A."
        assert record.references[0].citation == """\
...
PubMed=8352747"""
        assert record.references[1].number == "2"
        assert record.references[1].authors == "Henrissat B., Callebaut I., Fabrega S., Lehn P., Mornon J.-P., Davies G."
        assert record.references[1].citation == """\
...
PubMed=7624375"""
        assert record.references[2].number == "E1"
        assert record.references[2].authors == ""
        assert record.references[2].citation == "http://www.expasy.org/cgi-bin/lists?glycosid.txt"
    ...
    def test_read_pdoc0933(self):
        ...
        assert record.accession == "PDOC00933"
        assert len(record.prosite_refs) == 1
        assert record.prosite_refs[0] == ("PS01213", "GLOBIN_FAM_2")
        assert record.text == """\
**********************************************
...
"""
        ...
        assert len(record.references) == 4
        assert record.references[0].number == "1"
        assert record.references[0].authors == "Concise Encyclopedia Biochemistry, Second Edition, Walter de Gruyter, Berlin New-York (1988)."
        assert record.references[0].citation == ""
        assert record.references[1].number == "2"
        assert record.references[1].authors == "Takagi T."
        assert record.references[1].citation == "Curr. Opin. Struct. Biol. 3:413-418(1993)."
        assert record.references[2].number == "3"
        assert record.references[2].authors == "Couture M., Chamberland H., St-Pierre B., Lafontaine J., Guertin M."
        assert record.references[2].citation == """\
...
PubMed=8177215"""
        assert record.references[3].number == "4"
        assert record.references[3].authors == "Couture M., Das T.K., Savard P.Y., Ouellet Y., Wittenberg J.B., Wittenberg B.A., Rousseau D.L., Guertin M."
        assert record.references[3].citation == """\
...
class TestProdocParse(unittest.TestCase):
    ...
    def test_parse_pdoc(self):
        ...
        with open(filename) as handle:
            ...
            record = next(records)
            assert record.accession == "PDOC00000"
            assert len(record.prosite_refs) == 0
            assert record.text == """\
**********************************
...
"""
            ...
            assert len(record.prosite_refs) == 1
            assert record.prosite_refs[0] == ("PS00001", "ASN_GLYCOSYLATION")
            assert record.text == """\
************************
...
PubMed=6847620"""
            assert record.references[3].number == "4"
            assert record.references[3].authors == "Gavel Y., von Heijne G."
            assert record.references[3].citation == """\
...
PubMed=2349213"""
            assert record.references[4].number == "5"
            assert record.references[4].authors == "Miletich J.P., Broze G.J. Jr."
            assert record.references[4].citation == """\
...
PubMed=1694179"""
            ...
            record = next(records)
            assert record.accession == "PDOC00004"
            assert len(record.prosite_refs) == 1
            assert record.prosite_refs[0] == ("PS00004", "CAMP_PHOSPHO_SITE")
            assert record.text == """\
****************************************************************
...
"""
            ...
            assert record.references[0].number == "1"
            assert record.references[0].authors == "Fremisco J.R., Glass D.B., Krebs E.G."
            assert record.references[0].citation == """\
J. Biol. Chem. 255:4240-4245(1980)."""
            assert record.references[1].number == "2"
            assert record.references[1].authors == "Glass D.B., Smith S.B."
            assert record.references[1].citation == """\
...
PubMed=6317673"""
            assert record.references[2].number == "3"
            assert record.references[2].authors == "Glass D.B., el-Maghrabi M.R., Pilkis S.J."
            assert record.references[2].citation == """\
...
PubMed=3005275"""
            ...
            record = next(records)
            assert record.accession == "PDOC60030"
            assert len(record.prosite_refs) == 1
            assert record.prosite_refs[0] == ("PS60030", "BACTERIOCIN_IIA")
            assert record.text == """\
******************************************
...
"""
            ...
            assert record.references[0].number == "1"
            assert record.references[0].authors == "Ennahar S., Sonomoto K., Ishizaki A."
            assert record.references[0].citation == """\
...
PubMed=16232543"""
            assert record.references[1].number == "2"
            assert record.references[1].authors == "Johnsen L., Fimland G., Nissen-Meyer J."
            assert record.references[1].citation == """\
...
PubMed=15611086; DOI=10.1074/jbc.M412712200"""
            assert record.references[2].number == "3"
            assert record.references[2].authors == "Fimland G., Johnsen L., Dalhus B., Nissen-Meyer J."
        assert record.references[2].citation == """\
