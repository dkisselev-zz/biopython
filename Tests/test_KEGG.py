...
class EnzymeTests(unittest.TestCase):
    ...
    def test_sample(self):
        ...
        assert records[0].entry == "1.1.1.1"
        ...
        assert records[-1].entry == "2.7.2.1"
        assert str(records[-1]).replace(" ", "").split("\n")[:10] == [
                "ENTRYEC2.7.2.1",
                "NAMEacetatekinase",
                "acetokinase",
                "AckA",
    ...
    def test_irregular(self):
        ...
        assert records[0].entry == "1.14.18.1"
        assert records[-1].entry == "3.4.21.50"
    ...
    def test_new(self):
        ...
        assert records[0].entry == "6.2.1.25"
    ...
    def test_4letter(self):
        ...
        with open("KEGG/enzyme.4letter") as handle:
            records = list(Enzyme.parse(handle))
            assert len(records) == 1
        assert records[0].entry == "5.4.2.2"
        assert records[0].genes[0] == ("HSA", ["5236", "55276"])
        assert records[0].genes[8] == ("CSAB", ["103224690", "103246223"])
    ...
    def test_exceptions(self):
        ...
        with open("KEGG/enzyme.sample") as handle:
            ...
            assert "More than one record found in handle" in str(context.value)
...
class CompoundTests(unittest.TestCase):
    ...
    def test_sample(self):
        ...
        assert len(records) == 8
        assert records[1].entry == "C00017"
        assert records[1].mass == ""
        assert records[1].formula == "C2H4NO2R(C2H2NOR)n"
        assert records[1].name == ["Protein"]
        assert records[1].pathway == [("PATH", "map00450", "Selenocompound metabolism")]
        assert len(records[1].enzyme) == 21
        assert records[1].enzyme[0] == ("2.3.2.6")
        assert records[1].structures == []
        assert records[1].dblinks[0] == ("PubChem", ["3319"])
        assert str(records[-1]).replace(" ", "").split("\n")[:11] == [
                "ENTRYC01386",
                "NAMENH2Mec",
    ...
    def test_irregular(self):
        ...
        with open("KEGG/compound.irregular") as handle:
            records = list(Compound.parse(handle))
        assert len(records) == 2
        assert records[0].entry == "C01454"

    def test_mass(self):
        """record.mass tests."""
        with open("KEGG/compound.sample") as handle:
            records = list(Compound.parse(handle))
        assert records[0].entry == "C00023"
        assert records[0].mass == "55.9349"
        assert records[2].entry == "C00099"
        assert records[2].mass == "89.0477"
        assert records[3].entry == "C00294"
        assert records[3].mass == "268.0808"
...
class MapTests(unittest.TestCase):
    ...
    def test_map00950(self):
        ...
        assert (str(rxs[0]) == "(R)-N-Methylcoclaurine + (S)-Coclaurine + NADPH + O2 "
            "<=> 2'-Norberbamunine + 2 H2O + NADP")
        assert str(rxs[-1]) == "Tyramine <=> Dopamine"
