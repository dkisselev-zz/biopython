...
class TestXdna(unittest.TestCase):
    ...
    def test_read(self):
        ...
        for sample in self.sample_data.values():
            ...
            assert sample["molecule_type"] == record.annotations["molecule_type"]
            ...
            for i in range(len(sample["features"])):
                ...
                assert exp_feat["label"] == read_feat.qualifiers["label"][0]
...
class TestInvalidXdna(unittest.TestCase):
    ...
    def test_unsupported_version(self):
        ...
        with pytest.raises(ValueError, match="Unsupported XDNA version"):
            SeqIO.read(h, "xdna")
        h.close()
    ...
    def test_corrupted_length(self):
        ...
        with pytest.raises(ValueError, match="Cannot read 2048 bytes from handle"):
            SeqIO.read(h, "xdna")
        h.close()
    ...
    def test_missing_features(self):
        ...
        with pytest.raises(ValueError, match="Cannot read 1 bytes from handle"):
            SeqIO.read(h, "xdna")
        h.close()
...
class TestXdnaWriter(unittest.TestCase):
    ...
    def test_warnings_on_data_loss(self):
        ...
        with pytest.warns(BiopythonWarning, match="Too many features"):
            SeqIO.write([record], h, "xdna")

        # Now a record with a fuzzy-located feature
        feature = SeqFeature(SimpleLocation(BeforePosition(2), 3), type="misc_feature")
        record.features = [feature]
        with pytest.warns(BiopythonWarning, match=r"Dropping \d+ features with fuzzy locations"):
            SeqIO.write([record], h, "xdna")
        ...
        record.features = [feature]
        with pytest.warns(BiopythonWarning, match="Some annotations were truncated to 255 characters"):
            SeqIO.write([record], h, "xdna")
