...
try:
    ...
    pytest.skip("Install igraph if you wish to use it with Bio.Phylo", allow_module_level=True)
...
class UtilTests(unittest.TestCase):
    ...
    def test_to_igraph_simple(self):
        ...
        assert graph.vs["name"] == ["elem_1", "elem_2", "elem_3"]
        assert graph.es["width"] == ["width_1", "width_2"]
    ...
    def test_to_igraph_exd_ollo(self):
        ...
        assert graph.vcount() == 659
        assert graph.ecount() == graph.vcount() - 1
