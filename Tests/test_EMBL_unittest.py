...
class EMBLTests(unittest.TestCase):
    def test_embl_content_after_co(self):
        ...
        try:
            ...
            assert str(e) == "Unexpected content after SQ or CO line: 'XX'"
        else:
            assert False, "Error message without explanation raised by content after CO line"
    ...
    def test_embl_0_line(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert len(w) == 0, ("Unexpected parser warnings: "
    ...
    def test_embl_no_coords(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert w, "Expected parser warning"
            assert [str(_.message) for _ in w] == ["EMBL sequence line missing coordinates"] * 3
            assert len(rec) == 154
            assert rec.seq[:10] == "MVLSEGEWQL"
            assert rec.seq[-10:] == "AKYKELGYQG"
    ...
    def test_embl_wrong_dr_line(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert w, "Expected parser warning"
            assert [str(_.message) for _ in w] == ["Malformed DR line in EMBL file."]
