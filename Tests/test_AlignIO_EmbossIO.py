...
class TestEmbossIO(unittest.TestCase):
    def test_pair_example(self):
        ...
        assert [r.id for r in alignments[0]] == ["IXI_234", "IXI_235"]

    def test_simple_example(self):
        alignments = list(EmbossIterator(StringIO(simple_example)))
        assert len(alignments) == 1
        assert len(alignments[0]) == 4
        assert [r.id for r in alignments[0]] == ["IXI_234", "IXI_235", "IXI_236", "IXI_237"]
    ...
    def test_pair_plus_simple(self):
        ...
        assert [r.id for r in alignments[0]] == ["IXI_234", "IXI_235"]
        assert [r.id for r in alignments[1]] == ["IXI_234", "IXI_235", "IXI_236", "IXI_237"]

    def test_pair_example2(self):
        alignments = list(EmbossIterator(StringIO(pair_example2)))
        assert len(alignments) == 5
        assert len(alignments[0]) == 2
        assert [r.id for r in alignments[0]] == ["ref_rec", "gi|94968718|receiver"]
        assert [r.id for r in alignments[4]] == ["ref_rec", "gi|94970041|receiver"]

    def test_pair_example3(self):
        alignments = list(EmbossIterator(StringIO(pair_example3)))
        assert len(alignments) == 1
        assert len(alignments[0]) == 2
        assert [r.id for r in alignments[0]] == ["asis", "asis"]
